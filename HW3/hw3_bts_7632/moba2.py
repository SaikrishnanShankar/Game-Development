...
HUNTERFIRERATE = BASEFIRERATE//4
HUNTERBULLETDAMAGE = BASEBULLETDAMAGE * 2
HUNTERBULLETRANGE = BASEBULLETRANGE * 2
HUNTERBULLETSPEED = BASEBULLETSPEED
BASEBULLET = "sprites/bullet2.gif"
HUNTERBULLET = "sprites/bullet.gif"
SPAWNNUM = 3
MAXSPAWN = 4
AREAEFFECTDAMAGE = 25
AREAEFFECTRATE = 60
...
class MOBABullet(Bullet):
	...
	def hit(self, thing):
		...
		if isinstance(thing, Hero) and (thing.getTeam() is not None and self.owner.getTeam() is not None and thing.getTeam() != self.owner.getTeam()):
...
class HunterBullet(MOBABullet):
	...
	def __init__(self, position, orientation, world):
		MOBABullet.__init__(self, position, orientation, world, HUNTERBULLET, HUNTERBULLETSPEED, HUNTERBULLETDAMAGE, HUNTERBULLETRANGE)
...
class Base(Mover):
	...
	def spawnHunter(self, angle = 0.0):
		if self.hunterType is not None:
			vector = (math.cos(math.radians(angle)), -math.sin(math.radians(angle)))
			agent = self.hunterType(self.getLocation(), 0, self.world, bulletclass=HunterBullet, firerate = HUNTERFIRERATE)
			pos = (vector[0]*(self.getRadius()+agent.getRadius())/2.0,vector[1]*(self.getRadius()+agent.getRadius())/2.0)
			agent.move(pos)
			if self.nav is not None:
				newnav = cloneAStarNavigator(self.nav)
				agent.setNavigator(newnav)
			agent.setTeam(self.getTeam())
			agent.setOwner(self)
			self.world.addNPC(agent)
			agent.start()
			return agent
