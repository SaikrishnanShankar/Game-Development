...
class Action(object):
  ...
  def execute(self, delta = 0):
    ...
    if self.first:
      ...
      self.first = False
    if ret:
      self.agent.update_world(self.build_add_list(), self.build_delete_list())
    return ret
  ...
  def enter(self):
    if self.agent is not None:
      ...
      return self.agent.check_preconditions(self.build_preconditions())
    else:
      return False
  ...
  def build_preconditions(self):
    built = set()
    for p in list(self.preconditions):
      built.add(p.replace('{name}', self.agent.name))
    return built

  def build_add_list(self):
    built = set()
    for p in list(self.add_list):
      built.add(p.replace('{name}', self.agent.name))
    return built

  def build_delete_list(self):
    built = set()
    for p in list(self.delete_list):
      built.add(p.replace('{name}', self.agent.name))
    return built
...
class InstantAction(Action):
  ...
  def execute(self, delta = 0):
    ret = Action.execute(self, delta)
    self.exit()
    return ret

class TriggerAction(InstantAction):
  
  def execute(self, delta = 0):
    ret = InstantAction.execute(self, delta)
    if ret:
      self.agent.trigger(self)
    return ret

class DoorAction(TriggerAction):
  
  def execute(self, delta = 0):
    return TriggerAction.execute(self, delta)
