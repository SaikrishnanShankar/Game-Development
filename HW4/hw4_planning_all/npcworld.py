...
class NPCAgent(Agent, Planner):
  ...
  def __init__(self, image, position, orientation, speed, world, hitpoints = HITPOINTS, firerate = FIRERATE, bulletclass = Bullet, name = 'agent'):
...
class Place(Thing):
  ...
  def __init__(self, name, label, position, width, height, world, color = (0, 0, 0), linewidth = 1):
    self.world = world                 # pointer to world object
    self.name = name
  ...
  def draw(self, parent):
    if self.surface != None:
      ...
      # The name (label)
      label = self.font.render(self.name, True, (0, 0, 0))
  ...
  def agent_here(self, agent):
    return (agent.getLocation()[0] > self.position[0]) and (agent.getLocation()[0] < self.position[0] + self.width) and (agent.getLocation()[1] > self.position[1]) and (agent.getLocation()[1] < self.position[1] + self.height)
  ...
  def update(self, delta = 0):
    #### TODO ITERATE THROUGH NPCS
    if self.agent_here(self.world.agent):
      self.world.add_proposition_to_world_state(self.build_label(self.world.agent))
    else:
      self.world.remove_proposition_from_world_state(self.build_label(self.world.agent))

  def build_label(self, agent):
    return self.label.replace('{name}', agent.name)
...
class DoorPlace(Place):
  ...
  def __init__(self, name, label, position, width, height, world, p1, p2, sprite = GATE, color = (0, 0, 0), linewidth = 1):
    Place.__init__(self, name, label, position, width, height, world, color, linewidth)
...
class NPCWorld(GatedWorld):
  ...
  def update_world(self, add, delete):
    for a in add:
      self.add_proposition_to_world_state(a)
    for d in delete:
      self.remove_proposition_from_world_state(d)
  ...
  ### Trigger an action
  def trigger(self, action): #### Need to check the presence of the agent that triggered
    for place in self.places:
      # Is the agent in a place where it is legal to trigger this action?
      if place.agent_here(action.agent) and action.name in [t.name for t in place.possible_triggers]:
        place.trigger(action)
        # Record the fact that it happened
        place.actions_performed_here.append(action.name)
        # Update the world state
        self.world_state = self.world_state.difference(action.build_delete_list()).union(action.build_add_list())
        return True
    return False
  ...
  def doKeyDown(self, key):
    if key == 116: #'t'
      for place in self.places:
        if place.agent_here(self.agent):
          for trigger in place.possible_triggers:
            trigger.agent = self.agent
            print("try triggering", trigger.name)
            trigger.execute()
    elif key == 119: #w
      print("WORLD STATE", self.world_state)
    elif key == 112: #p
      for npc in self.getNPCs():
        if isinstance(npc, Planner):
          init_state = State(propositions=list(self.world_state))
          npc.the_plan, visited = npc.astar(init_state, npc.goal_state, npc.actions) 
          print("the plan", [a.name for a in npc.the_plan])
